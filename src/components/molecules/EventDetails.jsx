...
const EventDetails = ({ event: propEvent }) => {
  ...
  const photoUrl = photos[0] || "https://via.placeholder.com/800";
        ...
        <div>
          ...
          <div className="mt-4 text-lg">
            <p className="text-white/60">
              Date: {dateTime ? new Date(dateTime).toLocaleString() : "No Date"}
            </p>
            <p className="text-white/60">Location: {city.name || "No City"}</p>
            <p className="text-white/60">Category: {category.categoryName || "No Category"}</p>
          </div>
        </div>
